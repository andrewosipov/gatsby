...
class USAMap extends React.Component {
  ...
  buildPaths = () => {
    ...
    for (let stateKey in data) {
        const opacity = ( this.props.customize && this.props.customize[stateKey] && this.props.customize[stateKey].opacity ) ? this.props.customize[stateKey].opacity : 0.7 ;
      const path = <USAState key={stateKey} dimensions={data[stateKey]["dimensions"]} state={stateKey} opacity={opacity} fill={this.fillStateColor(stateKey)} onClickState={this.stateClickHandler(stateKey)} />
      paths.push(path);
    };
    return paths;
  };
